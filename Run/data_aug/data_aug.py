...
def load_data(path_train_x, path_train_y, path_val_x, path_val_y, path_test_x, path_test_y):
    ...
    test_y  = sorted(glob(os.path.join(path_test_y,'**', '*.bmp'),recursive=True))
...
def augment_data(images, masks, save_path, augment=True):
    ...
    for idx, (x, y) in tqdm(enumerate(zip(images, masks)), total=len(images)):
        ...
        if augment:
            ...
            aug = Rotate(limit=45, p=1.0,border_mode=cv2.BORDER_CONSTANT,value=0,mask_value=(255,255,255))
            augmented = aug(image=x, mask=y)
            x3 = augmented["image"]
            y3 = augmented["mask"]
...
if __name__ == "__main__":
    ...
    augment_data(train_x, train_y, "/home/kebl6872/Desktop/new_data/REFUGE2/train/", augment=True)
